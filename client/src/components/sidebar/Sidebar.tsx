...
function Sidebar() {
  ...
  function handleFilter(category: CrimeType) {
    ...
    if (category === "All") {
      ...
      const filteredCrimes = crimes.filter(crime => crime.crime_type === category);
  ...
  return (
    ...
    {showSidebar ? (
      ...
      <div className="flex gap-2 justify-end">
        <p className='mr-auto text-white font-bold'>911 - <span className='font-normal'>Santiago Streets</span></p>
        <SortMenu selectedSort={selectedSort} setSelectedSort={setSelectedSort}></SortMenu>
        <FilterMenu setSelectedFilter={(category: CrimeType) => handleFilter(category)} selectedFilter={selectedFilter} />
        {viewportWidth <= 768 && 
          <button onClick={()=>setShowSidebar(!showSidebar)} className="z-50 bg-primary text-white p-1 rounded-full">
      ...
      {filteredCrimes?.map((crime: Crime, index:number) => (
        <SidebarCard key={crime._key} crime={crime} index={index}/>
      ))}
      </div>
      </section>):<button onClick={()=>setShowSidebar(!showSidebar)} className="absolute z-50 bg-primary text-white top-[1rem] left-[1rem] p-1 rounded-full">
